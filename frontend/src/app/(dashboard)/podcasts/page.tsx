...
import { DistributionTab } from '@/components/podcasts/DistributionTab'
import { Mic, LayoutTemplate, Radio } from 'lucide-react'
...
export default function PodcastsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'episodes' | 'templates' | 'distribution'>(
    'episodes'
  )
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="px-6 py-6 space-y-6">
          ...
          <Tabs
            value={activeTab}
            onValueChange={(value) =>
              setActiveTab(value as 'episodes' | 'templates' | 'distribution')
            }
            className="space-y-6"
          >
            <div className="space-y-2">
              <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t('podcasts.chooseAView')}</p>
              <TabsList aria-label={t('common.accessibility.podcastViews')} className="w-full max-w-xl">
